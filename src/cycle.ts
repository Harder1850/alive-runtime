 ...
 *
 * The ONLY place requestCognition is called. Not exported.
 * Signal must pass STG before mind is invoked — no exceptions.
 *
 * Full cycle (v16 §6 Canonical Pipeline):
 *   1. Firewall guard
 *   2. CB → populate signal.novelty
 *   3. STG → OPEN / DEFER / DENY
 *   4. Cognition (synthesize) — OPEN only
 *   5. Admissibility check
 *   6. Execute authorized action
 *   7. Experience Stream cycle record
 *   8. LTG stub
 *   9. ASM update
 *
 * Invariants enforced:
 *   INV-001: No cognition without STG OPEN
 *   INV-006: Firewall-blocked signals rejected before STG
...
const asm: ASMState = { cpu_risk: 0, disk_risk: 0, mode: 'idle', cycleCount: 0, lastUpdated: Date.now() };
const getASM  = (): ASMState => ({ ...asm });
const updateASM = (p: Partial<ASMState>) => Object.assign(asm, p, { lastUpdated: Date.now() });

// ─── LTG stub — Slice 3 replaces this ────────────────────────────────────────
...
export async function runCycle(signal: Signal): Promise<CycleResult> {
  ...
  // Guard: firewall must have cleared this signal
  if (signal.firewall_status !== 'cleared') {
    return { signal_id: signal.id, stg_result: 'DENY', stg_reason: `Firewall status '${signal.firewall_status}'`, executed: false, asm_after: getASM(), experience_stream: getExperienceStreamPath() };
  }

  // Step 1: Log signal receipt
  logSignalReceived(signal.id, { kind: signal.kind, source: signal.source, urgency: signal.urgency, confidence: signal.confidence, quality_score: signal.quality_score, raw_content: String(signal.raw_content).slice(0, 200) });
  ...
  const stgResult  = evaluateSTG(signalWithNovelty, {
  ...
  const stgReason =
    ...
    stgResult === 'DEFER' ? `cpu_risk=${currentASM.cpu_risk.toFixed(2)} ≥ 0.7 → deferred` : 'pre-check failed';

  logStgDecision(signal.id, stgResult, stgReason, { cpu_risk: currentASM.cpu_risk, urgency: signal.urgency, deltaScore: cbResult.deltaScore });
  console.log(`[STG] ${stgResult} — ${stgReason}`);

  if (stgResult !== 'OPEN') {
    return { signal_id: signal.id, stg_result: stgResult, stg_reason: stgReason, executed: false, asm_after: getASM(), experience_stream: getExperienceStreamPath() };
  }
  ...
  try {
    ...
    return { signal_id: signal.id, stg_result: 'OPEN', stg_reason: stgReason, executed: false, asm_after: getASM(), experience_stream: getExperienceStreamPath() };
  }
  ...
  if (admissibility.status === 'blocked') {
    return { signal_id: signal.id, stg_result: 'OPEN', stg_reason: stgReason, synthesizer_level: candidate.level, admissibility: 'blocked', executed: false, asm_after: getASM(), experience_stream: getExperienceStreamPath() };
  }
  ...
  // Step 9: Cycle complete record
  logCycleComplete(signal.id, { duration_ms: Date.now() - startTime, signal_kind: signal.kind, signal_urgency: signal.urgency, stg: stgResult, synthesizer_level: candidate.level, admissibility: admissibility.status, action_type: candidate.action.type, executed: outcome.success, ltg: ltgVerdict, asm_cpu_risk: finalASM.cpu_risk, asm_cycle_count: finalASM.cycleCount });

  return { signal_id: signal.id, stg_result: 'OPEN', stg_reason: stgReason, synthesizer_level: candidate.level, rule_matched: candidate.level === 'rule' ? candidate.reason : undefined, admissibility: admissibility.status, executed: outcome.success, execution_detail: outcome.detail, ltg_verdict: ltgVerdict, asm_after: finalASM, experience_stream: getExperienceStreamPath() };
}
