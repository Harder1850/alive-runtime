...
const DEFER_TTL_MS     = 30_000;   // 30 s → starvation promotion threshold
const DEFER_MAX_AGE_MS = 90_000;   // 3× TTL → hard expiry
...
interface DeferredItem {
  ...
  priority:    number;
  deferredAt:  number;
  promoted:    boolean;   // true once starvation-promoted
}

class DeferQueue {
  private items: DeferredItem[] = [];
   ...
   */
  push(signal: Signal, priority: number): void {
    if (this.items.length >= MAX_QUEUE_SIZE) {
      ...
      console.log(
        `[STG-QUEUE] OVERFLOW — evicted signal=${evicted.signal.id.slice(0, 8)} ` +
    ...
    if (priority >= 3) {
      ...
      if (insertAt >= 0) {
        ...
        console.log(
          `[STG-QUEUE] INTERRUPT  signal=${signal.id.slice(0, 8)} ` +
    ...
    console.log(
      `[STG-QUEUE] ENQUEUE  signal=${signal.id.slice(0, 8)} ` +
  ...
  tick(): void {
    ...
    this.items = this.items.filter((i) => {
      ...
      if (age > DEFER_MAX_AGE_MS) {
        console.log(
          `[STG-QUEUE] EXPIRED  signal=${i.signal.id.slice(0, 8)} ` +
    ...
    for (const item of this.items) {
      ...
      if (age > DEFER_TTL_MS && !item.promoted) {
        ...
        promoted++;
        console.log(
          `[STG-QUEUE] PROMOTE  signal=${item.signal.id.slice(0, 8)} ` +
    ...
    if (purged > 0 || promoted > 0) {
      console.log(
        `[STG-QUEUE] TICK  purged=${purged} promoted=${promoted} remaining=${this.items.length}`,
      );
    }
  }

  /** Pop the highest-priority deferred item. Returns undefined if queue is empty. */
  pop(): DeferredItem | undefined {
    return this.items.shift();
  }

  /** Peek at the next item without removing it. */
  peek(): DeferredItem | undefined {
    return this.items[0];
  }
...
export const deferQueue = new DeferQueue();
...
export interface STGContext {
  triagePriority?: number;   // 0–4 from triage service (or 1–5 Slice 2)
