...
exports.triageSignal = triageSignal;
const flag_1 = require("../../../alive-constitution/contracts/flag");
const CONTENT_RULES = [
    // Critical threats
...
const ROUTE_ORDER = ['reflex', 'brain', 'defer', 'log_only'];
function pickHighestRoute(routes) {
...
function triageSignal(signal, cb) {
    const flags = [];
    const content = String(signal.raw_content ?? '');
    // --- CB anomaly flag ---
    if (cb.isAnomaly) {
        flags.push((0, flag_1.createFlag)({
            signal_id: signal.id,
    ...
    if (flags.length > 0) {
        console.log(`[TRIAGE] ${flags.length} flag(s) for signal="${content.slice(0, 50)}": ` +
